...
###########################################################################################
r = redis.StrictRedis(host = 'ec2-35-171-246-105.compute-1.amazonaws.com',
                      port = 6379)

@app.route('/')
@app.route('/top10')
def top10():
    cikCount_list_All = r.zrevrange('All', 0, 9, withscores = True)
...
conn       = psycopg2.connect(host     = 'post20g.cvvi4duxl8pw.us-east-1.rds.amazonaws.com',\
                              ...
                              password = "Cd2017Job")
cur        = conn.cursor()
cur.execute(""" SELECT * FROM Quantiles;""",)
quant      = cur.fetchall()
...
quan95     = list(map(lambda x: x[3] *1 0, quant_list))
...
def seconds():
    ...
    return render_template("seconds.html",
                           title      = 'seconds',
